...
interface InventoryTableScrollFadeProps {
  children: ReactNode;
  className?: string;
}

export function InventoryTableScrollFade({
  children,
  className,
}: InventoryTableScrollFadeProps) {
  const fade = useScrollFade("vertical", "[data-inventory-table-scroll]");

  return (
    <div ref={fade.containerRef} className={cn("relative min-h-0", className)}>
      <div data-inventory-table-scroll className="h-full min-h-0 overflow-auto">
        {children}
      </div>
      <div
        ref={fade.startRef}
